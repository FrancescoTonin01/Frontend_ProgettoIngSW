...
import { login, useAuth, type UserData } from '@/components/auth';
...
// Funzione per prendere i dati dell'utente
...
// Funzione per validare input password
    ...
    if (axios.isAxiosError(error)) {
      if (error.response) {
        switch (error.response.status) {
          case 400:
            errorMessage.value = t('badRequestError');
            break;
          case 401:
            errorMessage.value = t('invalidCurrentPassword');
            break;
          case 404:
            errorMessage.value = t('userNotFound');
            break;
          case 409:
            errorMessage.value = t('usernameAlreadyExists');
            break;
          case 500:
            errorMessage.value = t('internalServerError');
            break;
          default:
            errorMessage.value = `${t('error')} ${error.response.status}: ${error.response.data || t('unknownError')}`;
        }
